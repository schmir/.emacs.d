...
;; Copyright 2007, 2008, 2009 Tassilo Horn
;;
...
;;
;; If an  abbrev contains hyphens,  it may use  substrings instead of  only one
;; character per  word.  For example  "out-mi-mo" matches `outline-minor-mode'.
;; This allows you to type a bit more to make the abbrev match only the command
;; you have  in mind and  omit the query  prompt that pops  up if an  abbrev is
;; ambiguous.
;;
...
;;
;; To  make it  work faster,  `exec-abbrev-cmd'  caches the  list of  available
;; commands.  If you create  a new function, you have to hit  `C-.' an the exec
;; abbrev promt or execute `exec-abbrev-cmds-clear'  to rebuild it, so that the
;; new command will be recognized.
;;
...
;; `exec-abbrev-cmd-mode',  a global minor  mode that  does bookkeeping  of how
;; often  you invoke  a command  with `exec-abbrev-cmd',  so that  the  list of
;; commands  you have  to choose  from is  sorted by  the frequency  of command
;; invokation.  After  a while in most cases  `C-x x <abbrev> RET  RET' will do
;; what you want.
;;
...
(defvar exec-abbrev-cmds nil
  "A list of commands used to complete after typing TAB on the
exec abbrev prompt.  If it is nil, a list of all commands will be
created on the fly.")

(defun exec-abbrev-cmds ()
  "Return a list of all (cached) commands."
  (or exec-abbrev-cmds
      (let (c)
        (mapatoms
         (lambda (a)
        ...
        (setq exec-abbrev-cmds c))))

(defun exec-abbrev-cmds-update ()
  "Clear `exec-abbrev-cmds' so that it will be created anew,
taking new commands into account."
  (interactive)
  ;; (message "Updating `exec-abbrev-cmds'... %s" (format-time-string "(%T)"))
  (setq exec-abbrev-cmds nil
        exec-abbrev-cmds (exec-abbrev-cmds)))

(defvar exec-abbrev-cmds-timer (run-with-idle-timer 60 t 'exec-abbrev-cmds-update)
  "An idle timer which updates the list of commands everytime
emacs was idle for 60 seconds.  An update can be forced with
`C-.' at the `exec-abbrev-cmd' prompt, too.")
           ...
           (let* ((abbrev
                   (read-from-minibuffer
                    ...
                    (let ((map (copy-keymap minibuffer-local-map)))
                      ;; TAB escapes from the exec abbrev prompt and falls back
                      ;; to something like `execute-extended-command', copying
                      ;; the current minibuffer contents.
                      (define-key map "\C-i"
                        (lambda ()
                          (interactive)
                          (throw 'escape-from-exec-abbrev-cmd
                                 (minibuffer-contents))))
                      ;; C-. sets the cached command list to nil again, so that
                      ;; it will be created anew, taking new commands into
                      ;; account.
                      (define-key map (kbd "C-.") 'exec-abbrev-cmds-update)
                      map)))
                  (regexp
                   (concat "^"
