...
(setup emacs-mail
  ...
  (with-eval-after-load 'sendmail
    ...
    (message "setup-mail.el: configuring sendmail program")
    (when-let* ((msmtp (executable-find "msmtp")))
      (setq sendmail-program msmtp)))

  (with-eval-after-load 'message
    (message "setup-mail.el: configuring ecomplete")
    ;; ecomplete setup, see
  ...
  (with-eval-after-load 'mailcap
    (when-let* ((viewer (cond
      ...
      (message "setup-mail.el: using '%s' as external PDF viewer" viewer)
      (push `((viewer . ,viewer)
